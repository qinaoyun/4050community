$(function(){
  $('#dgList').datagrid({
        url: '../bulletinlist.do?_r='+Math.random(),
        fit: true,
        fitColumns: true,
        idField: 'id',
        loadMsg: '正在加载系统功能...',
...
//					        $.messager.alert('友情提示', rows[0].filetype + '系统暂时此类文件不支持自助打印', 'info');
					    window.location.href="../admin/bulletinadd.jsp";
//					}else{
			    ...
			    handler: function () {
					...
					if (rows.length > 0) {
                        ...
                        if(confirm("确认要删除该文件吗？"))
                        	window.location.href="../bulletindelete.do?id="+ rows[0].id+"";
					}else{
			    ...
			    handler: function () {
					...
					if (rows.length > 0) {
						...
						if (rows[0].filetype == 'doc' || rows[0].filetype == 'docx' || rows[0].filetype == 'pdf' || rows[0].filetype == 'jpg' || rows[0].filetype == 'xls' || rows[0].filetype == 'xlsx')
					    	...
					    	window.location.href="../bulletinview.do?id="+ rows[0].id+"";
					}else{
