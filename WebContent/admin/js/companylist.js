...
$(function(){
  $('#dgList').datagrid({
        url: '../companylist.do?_r='+Math.random(),
        fit: true,
        fitColumns: true,
        idField: 'id',
        loadMsg: '正在加载系统功能...',
        ...
        columns: [[
            ...
            { field: 'username', title: '用户名', width: 40, hidden: false },
			{ field: 'accounttype', title: '账户类型', width: 80 },
			{ field: 'email', title: 'Email', width: 40 },
			{ field: 'date', title: '注册日期', width: 50 }
...
//                        	window.location.href="../companydelete.do?id="+ rows[0].id+"";
//					}else{
			    ...
			    handler: function () {
					...
					if (rows.length > 0) {
						...
						if (rows[0].filetype == 'doc' || rows[0].filetype == 'docx' || rows[0].filetype == 'pdf' || rows[0].filetype == 'jpg' || rows[0].filetype == 'xls' || rows[0].filetype == 'xlsx')
					    	...
					    	window.location.href="../companyview.do?id="+ rows[0].id+"";
					}else{
